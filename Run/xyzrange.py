...
ANCHOR_POSITIONS = {
    1: (0.00, 0.00, 2.00),   # Anchor 1
    2: (10.00, 0.00, 2.00),  # Anchor 2
    3: (0.00, 10.00, 2.00),  # Anchor 3
    4: (10.00, 10.00, 2.00), # Anchor 4
    5: (5.00, 5.00, 4.00)    # Anchor 5 (Optional)
}

MIN_ANCHORS_FOR_3D = 4 # Minimum number of anchors required for 3D localization
def multilaterate_3d(ranges: dict, max_iterations=20, tolerance=1e-5):
    """
    Calculates the 3D position (X, Y, Z) and an estimated error radius 
    using an Iterative Least Squares Estimation (LSE) approximation.
    
    :param ranges: {aid: (filtered_distance, noise_sigma)}
    :return: (X, Y, Z, Range_Radius) or (None, None, None, None)
    """
    if len(ranges) < MIN_ANCHORS_FOR_3D:
    ...
    for aid, (d, sigma) in ranges.items():
        pos = ANCHOR_POSITIONS.get(aid)
        if pos and d > 0: # Ensure we have a position and a positive distance
            anchors.append(pos)
            distances.append(d)
            sigmas.append(max(sigma, 0.01)) # Prevent zero division
    
    if len(anchors) < MIN_ANCHORS_FOR_3D:
        return None, None, None, None

    X_a = [p[0] for p in anchors]
    Y_a = [p[1] for p in anchors]
    Z_a = [p[2] for p in anchors]
    R = distances
    W = [1.0 / (s**2) for s in sigmas] # Weights: 1 / variance

    # 1. Initial Guess: Average of Anchor positions
    initial_x = sum(X_a) / len(X_a)
    initial_y = sum(Y_a) / len(Y_a)
    initial_z = sum(Z_a) / len(Z_a)
    pos = [initial_x, initial_y, initial_z]

    # Iterative Least Squares (Simplified Gradient Step)
    final_error_vector = []
    for i in range(max_iterations):
        J = [] # Jacobian matrix
        f = [] # Error vector (predicted distance - measured distance)
        total_weight = sum(W)
        ...
        for j in range(len(anchors)):
            ...
            # Predicted range
            dist_pred = (dx**2 + dy**2 + dz**2)**0.5
            
            # Error (Residual)
            error = dist_pred - R[j]
            f.append(error)
            
            # Jacobian row: [ (x-xi)/di, (y-yi)/di, (z-zi)/di ]
            if dist_pred > 1e-6:
                J.append([dx / dist_pred, dy / dist_pred, dz / dist_pred])
            else:
                J.append([0, 0, 0])
        ...
        # Calculate weighted gradient (approximates J^T * W * f)
        grad_x, grad_y, grad_z = 0, 0, 0
        for j in range(len(anchors)):
            # Weighted Jacobian * Error
            grad_x += W[j] * J[j][0] * f[j]
            grad_y += W[j] * J[j][1] * f[j]
            grad_z += W[j] * J[j][2] * f[j]
            
        # Update position (step size controlled by 'step')
        step = 0.5 
        
        new_pos = [
            pos[0] - step * grad_x / total_weight,
            pos[1] - step * grad_y / total_weight,
            pos[2] - step * grad_z / total_weight,
        ]

        # Check for convergence
        delta_norm = ( (new_pos[0] - pos[0])**2 + (new_pos[1] - pos[1])**2 + (new_pos[2] - pos[2])**2 )**0.5
        pos = new_pos
        final_error_vector = f # Store last error vector
        
        if delta_norm < tolerance:
            break

    # Error Estimation (Range Radius)
    # Simple approximation: 2 * Average Absolute Residual Error
    avg_error = sum(abs(e) for e in final_error_vector) / len(final_error_vector) if final_error_vector else 0.0
    range_radius = avg_error * 2.0 

    return pos[0], pos[1], pos[2], range_radius
